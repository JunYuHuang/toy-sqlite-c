...
Cursor* leaf_node_find(
    Table* table, uint32_t page_num, uint32_t key
);
NodeType get_node_type(void* node);
void set_node_type(void* node, NodeType type);
void leaf_node_split_and_insert(
    Cursor* cursor, uint32_t key, Row* value
);
uint32_t get_unused_page_num(Pager* pager);
void create_new_root(
    Table* table, uint32_t right_child_page_num
);
uint32_t* internal_node_num_keys(void* node);
uint32_t* internal_node_right_child(void* node);
uint32_t* internal_node_cell(void* node, uint32_t cell_num);
uint32_t* internal_node_child(void* node, uint32_t child_num);
uint32_t* internal_node_key(void* node, uint32_t key_num);uint32_t get_node_max_key(void* node);
bool is_node_root(void* node);
void set_node_root(void* node, bool is_root);
void indent(uint32_t level);
void print_tree(
    Pager* pager, uint32_t page_num, uint32_t indentation_level
);
...
ExecuteResult execute_insert(Statement* statement, Table* table) {
    ...
    if (cursor->cell_num < num_cells) {
        uint32_t key_at_index = *leaf_node_key(
            node, cursor->cell_num
        );
...
void serialize_row(Row* source, void* destination) {
    ...
    memcpy(
        destination + USERNAME_OFFSET,
        &(source->username),
        USERNAME_SIZE
    );
    memcpy(
        destination + EMAIL_OFFSET,
        &(source->email),
        EMAIL_SIZE
    );
}
...
Pager* pager_open(const char* filename) {
    int fd = open(
        ...
        O_CREAT,    // Create file it does not exist
    ...
    if (pager->pages[page_num] == NULL) {
        ...
        if (page_num <= num_pages) {
            ...
            ssize_t bytes_read = read(
                pager->file_descriptor, page, PAGE_SIZE
            );
            if (bytes_read == -1) {
...
Cursor* table_start(Table* table) {
    ...
    void* root_node = get_page(
        table->pager, table->root_page_num
    );
...
void leaf_node_split_and_insert(
    ...
    Insert the new vlaue in one of the two nodes.
    Update parent or create a new parent.
    */

    void* old_node = get_page(
        cursor->table->pager, cursor->page_num
    );
    uint32_t new_page_num = get_unused_page_num(
        cursor->table->pager
    );
    void* new_node = get_page(
        cursor->table->pager, new_page_num
    );
   ...
   for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
        ...
        void* destination = leaf_node_cell(
...
void create_new_root(
    ...
    */
    ...
    void* right_child = get_page(
        table->pager, right_child_page_num
    );
    uint32_t left_child_page_num = get_unused_page_num(
        table->pager
    );
    void* left_child = get_page(
...
uint32_t* internal_node_key(void* node, uint32_t key_num) {
    return (
        internal_node_cell(node, key_num) +
...
uint32_t get_node_max_key(void* node) {
    switch (get_node_type(node)) {
        case NODE_INTERNAL:
            return *internal_node_key(
                node, *internal_node_num_keys(node) - 1
            );
        case NODE_LEAF:
            return *leaf_node_key(
                node, *leaf_node_num_cells(node) - 1
            );
    }
}
